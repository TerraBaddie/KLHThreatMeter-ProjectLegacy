...
me.testthreat = function()
	...
	elseif me.class == "shaman" then
		...
		mod.out.print(string.format(mod.string.get("print", "data", "multiplier"), me.lclass, "Project Legacy Shocks", me.mods.shaman.shockthreat))
...
me.mods = 
{
	...
	["shaman"] = 
	{
		...
		["shockthreat"] = 1.0,		-- Project Legacy Reverberation multiplier.
	}
}

-- Project Legacy does not publish Reverberation's per-rank threat values on the
-- class-change page. Read the live talent tooltip instead of guessing them.
-- If the tooltip cannot be read, the multiplier safely stays at 1.0.
me.projectlegacyreverberationrank = -1
me.projectlegacyreverberationmultiplier = 1.0
me.projectlegacytalenttooltip = nil

me.getprojectlegacyreverberation = function(rank)
	if (rank == nil) or (rank <= 0) then
		me.projectlegacyreverberationrank = 0
		me.projectlegacyreverberationmultiplier = 1.0
		return 1.0
	end

	if me.projectlegacyreverberationrank == rank then
		return me.projectlegacyreverberationmultiplier
	end

	if me.projectlegacytalenttooltip == nil then
		me.projectlegacytalenttooltip = CreateFrame("GameTooltip", "KLHTMProjectLegacyTalentTooltip", UIParent, "GameTooltipTemplate")
		me.projectlegacytalenttooltip:SetOwner(UIParent, "ANCHOR_NONE")
	end

	local tooltip = me.projectlegacytalenttooltip
	tooltip:ClearLines()
	tooltip:SetTalent(1, 7)

	local x
	local line
	local text
	local percent
	for x = 1, tooltip:NumLines() do
		line = getglobal("KLHTMProjectLegacyTalentTooltipTextLeft" .. x)
		if line then
			text = line:GetText()
			if text and string.find(string.lower(text), "threat") then
				local lowertext = string.lower(text)
				local startpos, endpos

				-- Prefer the percentage grammatically tied to "threat" so a second
				-- Nature-damage percentage on the same tooltip line cannot be mistaken for it.
				startpos, endpos, percent = string.find(lowertext, "threat.-by%s+(%d+)%%")
				if percent == nil then
					startpos, endpos, percent = string.find(lowertext, "(%d+)%%.-threat")
				end
				if percent == nil then
					startpos, endpos, percent = string.find(lowertext, "threat.-(%d+)%%")
				end
	...
	me.projectlegacyreverberationrank = rank
	if percent then
		me.projectlegacyreverberationmultiplier = math.max(0, 1.0 - percent / 100)
	else
		me.projectlegacyreverberationmultiplier = 1.0
	end
...
me.redomods = function()
	...
	if mod.my.class == "warrior" then
		...
		-- Project Legacy: Improved Heroic Strike increases Heroic Strike damage instead
		-- of reducing its Rage cost. The extra damage is already present in the combat log.
		me.mods.warrior.heroicstrikecost = 0.0
	...
	elseif mod.my.class == "shaman" then
		...
		-- Project Legacy: Reverberation reduces threat generated by Shock spells.
		rank = mod.data.gettalentrank("reverberation")
		me.mods.shaman.shockthreat = me.getprojectlegacyreverberation(rank)
	...
	elseif mod.my.class == "paladin" then
		...
		if mod.data.isbuffpresent("Interface\\Icons\\Spell_Holy_SealOfFury") then
			-- Project Legacy uses the Vanilla Righteous Fury rule: +60% Holy threat.
			-- Improved Righteous Fury increases that +60% bonus by 16/33/50%,
			-- yielding total Holy multipliers 1.696 / 1.798 / 1.900.
